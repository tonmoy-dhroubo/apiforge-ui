        ...
        <CardContent>
          <Tabs defaultValue="api" className="space-y-4">
            <TabsList>
              ...
              <TabsTrigger value="content">Content Permissions</TabsTrigger>
            </TabsList>
            <TabsContent value="api" className="space-y-4">
              <Card className="border border-border/60 bg-background/70">
                <CardHeader>
                  <CardTitle className="text-sm">Create API Permission</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="grid gap-3 md:grid-cols-3">
                    <Select
                      value={apiForm.contentTypeApiId || undefined}
                      onValueChange={(value) =>
                        setApiForm((prev) => ({
                          ...prev,
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setApiForm((prev) => ({ ...prev, method: value }))
                      }
                    >
                      <SelectTrigger>
                  ...
                  <RolePicker
                    ...
                    onChange={(roles) =>
                      setApiForm((prev) => ({ ...prev, allowedRoles: roles }))
                    }
                  />
                  <Button onClick={handleCreateApi}>Save API Permission</Button>
                </CardContent>
              </Card>
                ...
                <TableBody>
                  {apiPermissions.map((permission) => (
                    <TableRow key={permission.id}>
                      <TableCell>{permission.contentTypeApiId}</TableCell>
            ...
            <TabsContent value="content" className="space-y-4">
              <Card className="border border-border/60 bg-background/70">
                <CardHeader>
                  <CardTitle className="text-sm">Create Content Permission</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="grid gap-3 md:grid-cols-2">
                    <Select
                      value={contentForm.contentTypeApiId || undefined}
                      onValueChange={(value) =>
                        setContentForm((prev) => ({
                          ...prev,
                          contentTypeApiId: value,
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setContentForm((prev) => ({ ...prev, action: value }))
                      }
                    >
                      <SelectTrigger>
                  ...
                  {contentPermissions.length === 0 && (
                    <TableRow>
                      <TableCell
                        ...
                        className="text-center text-sm text-muted-foreground"
                      >
                        No content permissions configured.
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
